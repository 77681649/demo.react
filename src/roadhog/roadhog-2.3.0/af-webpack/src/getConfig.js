...
export default function getConfig(opts = {}) {
  ...
  const outputPath = opts.outputPath ? resolve(opts.cwd, opts.outputPath) : resolve(opts.cwd, 'dist')
  ...
  babelOptions.plugins = [
    ...(babelOptions.plugins || []),
    ...(opts.disableDynamicImport ? [require.resolve('babel-plugin-dynamic-import-node-sync')] : [])
  ]
  const babelUse = [
    {
  ...
  const config = {
    ...
    resolve: {
      modules: [resolve(__dirname, '../node_modules'), 'node_modules', ...(opts.extraResolveModules || [])],
      extensions: [
    ...
    module: {
      rules: [
        ...
        ...(opts.extraBabelIncludes || []).map(include => {
          return {
            test: /\.(js|jsx)$/,
            include: typeof include === 'string' ? join(opts.cwd, include) : include,
            use: babelUse
          }
        }),
        {
    ...
    plugins: [
      ...
      ...(opts.ignoreMomentLocale ? [new webpack.IgnorePlugin(/^\.\/locale$/, /moment$/)] : []),
      ...commonsPlugins,
      ...copyPlugins,
      ...(process.env.ANALYZE
        ? [
